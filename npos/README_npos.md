...
The paper starts with the standard open-world problem: a classifier is trained on a known distribution $P\_{\text{in}}$, but once deployed it can encounter samples from unknown classes. A conventional classifier has no explicit mechanism saying “I have never seen anything like this,” so it may still assign one of the known labels with high confidence.
...
The introduction already describes $R\_{\text{open}}$ conceptually as a form of **\*\*level-set estimation\*\*** that separates ID from OOD.
...
D\_{\text{in}} = \\{(x\_i, y\_i)\\}\_{i=1}^{n}

$$

There is no $D\_{\text{out}}$.
...
P\_{\text{in}}(z)\downarrow.

$$

It isn't literally computing the probability density $P\_{\text{in}}$ ; it is using neighbour geometry as a surrogate.
...
Once they find a boundary feature $h(x\_i)$, they generate candidates:
